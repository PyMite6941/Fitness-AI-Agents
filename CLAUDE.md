...
- **`user_integrations`** holds OAuth tokens (Strava/Fitbit). **`device_tokens`** = hashed phone-pairing tokens. **`coach_plans`** = AI Coach plans (JSONB `plan`, RLS on). **`ai_usage`** = per-user/day AI call counter for rate limiting. **`push_subscriptions`** = browser web-push subscriptions (unique `endpoint`, JSONB `subscription`) for the daily insight notification.
...
`/watch`, `/ingest` (generic device ingestion, any wearable — accepts a device token), `/routes` (gps.py), `/user` (incl. `/user/sources` and `DELETE /user/data` = GDPR erase), `/charts`, `/analyze` (heavy CrewAI — **503 on Vercel**, runs in the demo Space / locally), `/integrations`, `/demo`, `/device` (phone pairing tokens), `/coach` (AI Coach plans), `/insights` (Readiness + Watchdog), `/chat` (chat with your data), `/push` (web-push subscribe/unsubscribe/test), `/cron` (scheduler-only jobs), `/health`.

### Daily insights push (the retention loop)
A once-daily browser **push notification** delivering each opted-in user's Readiness score + top Watchdog alert. Pieces: `routes/push.py` (subscribe/unsubscribe/status/test), `routes/cron.py` `POST /cron/daily-insights` (guarded by `CRON_SECRET`, **not** Clerk — called by a scheduler), `push_utils.py` (VAPID web push via `pywebpush`), and reusable `readiness_for`/`alerts_for`/`build_daily_notification` in `routes/insights.py`. Frontend: `sw.js` push/notificationclick handlers, `lib/push.js`, and the `NotifyToggle` control on the Coach page. Scheduler = `.github/workflows/daily-insights.yml` (GitHub Actions cron, avoids touching the zero-config Vercel setup). **Setup/env in `backend/DAILY-INSIGHTS-PUSH.md`** (VAPID keys, `CRON_SECRET`, schema migration). Degrades gracefully: no VAPID keys → endpoints report `configured:false` and the toggle hides itself.
...
`main.jsx` wires Clerk + react-router. **Protected** routes: `/dashboard`, `/routes`, `/log`, `/coach` (AI Coach + Readiness + Watchdog hub, with the `NotifyToggle` daily-alert opt-in), `/chat` (chat with your data). **Public**: `/` (landing), `/demo` (no-auth sample dashboard), `/app` (PWA install + Android download), `/privacy` (policy + data-delete button). The Dashboard sidebar links to Coach/Chat/Routes/Log and shows an onboarding card when empty. All API calls go through `src/lib/api.ts`; `VITE_API_URL` points at the Vercel backend. The whole app is an installable **PWA** (manifest + service worker + heartbeat icon) and supports **web push** (`sw.js` push handlers + `lib/push.js`) for the daily insight notification.
